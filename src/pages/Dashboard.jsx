  ...
  return (
    <div className="flex flex-col gap-8 anim-in">
      <div>
        ...
        <h1 className="page-title">Noisy Character Recognition</h1>
        <p className="page-sub">Current phase: OpenCV Base Pipeline with Mock Recognizer. No Trained Model Yet.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <StatusCard label="Backend" value={backendOnline ? 'Online' : 'Offline'} tone={backendOnline ? 'good' : 'bad'} />
        <StatusCard label="Current pipeline" value="OpenCV Base" />
        <StatusCard label="Recognizer" value="Mock" />
        <StatusCard label="Dataset" value={datasetStatus?.downloaded ? 'Downloaded' : 'Not downloaded'} />
        <StatusCard label="Processed images" value={history.length} />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
        <div className="card flex flex-col h-full">
          <h3 className="card-title">Workspace</h3>
          <p className="card-sub mb-6">Run the real backend OpenCV pipeline and inspect generated artifacts.</p>
          <button
          ...
          >
            <span className="text-teal font-bold text-lg">Open Image Processing</span>
            <span className="text-muted text-sm mt-1">Upload and run OpenCV Base Pipeline</span>
          </button>
        </div>
                ...
                <tbody>
                  {recent.map((item) => (
                    <tr key={item.result_id}>
                      ...
                      <td><span className="badge badge-teal">{item.detected_boxes}</span></td>
      ...
      <div className="card">
        <h3 className="card-title">Model accuracy: Not available</h3>
        <p className="card-sub mt-2">No trained model yet. Current confidence values come from a deterministic mock recognizer, not a measured model.</p>
      </div>
    </div>
  );
};
