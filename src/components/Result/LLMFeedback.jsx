...
const LLMFeedback = ({ comment, modelVersion, mode = 'OpenCV Base Pipeline', datasetStatus = 'Dataset Not Downloaded' }) => {
  return (
    <div className="rounded-2xl p-5" style={{ background: 'rgba(13,148,136,0.08)', border: '1px solid rgba(45,212,191,0.22)', borderLeft: '3px solid #2dd4bf' }}>
      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: '#94a3b8' }}>{comment || 'Run the pipeline to generate feedback.'}</p>
        </div>
        <span className="badge badge-teal">Mock Recognizer</span>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <Meta label="Model version" value={modelVersion || 'opencv-cc-mock-recognizer-v1'} />
        <Meta label="Current mode" value={mode} />
        <Meta label="Dataset status" value={datasetStatus} />
      </div>
    </div>
  );
};
