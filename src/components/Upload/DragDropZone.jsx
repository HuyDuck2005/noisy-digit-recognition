...
const DragDropZone = ({ onImageSelect }) => {
  ...
  const validateAndPassFile = (file) => {
    ...
    if (!validTypes.includes(file.type)) {
      setError('File không hợp lệ. Vui lòng chọn ảnh JPG hoặc PNG.');
      return;
    }
    ...
    <div className="w-full">
      ...
      >
        ...
        <div
          ...
          style={{
            background: dragActive ? 'rgba(45,212,191,0.2)' : 'rgba(56,189,248,0.08)',
            border: `1px solid ${dragActive ? 'rgba(45,212,191,0.5)' : 'rgba(56,189,248,0.2)'}`,
          }}
        >
          <svg
            className="w-8 h-8 transition-transform"
            style={{ color: dragActive ? '#2dd4bf' : '#38bdf8', transform: dragActive ? 'scale(1.1)' : 'scale(1)' }}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        <p className="text-xs" style={{ color: '#475569' }}>Hỗ trợ JPG, PNG — tối đa 10MB</p>
